...
1. **No `setTimeout` / `setInterval`** – Use Angular signals (`signal`, `computed`, `effect`), `afterRenderEffect`, or RxJS operators (`timer`, `delay`, `debounceTime`) instead of raw timer APIs. They break zone-less change detection, are hard to test, and are never necessary in modern Angular.
...
6. **No `ViewEncapsulation.None`** – Always use the default `Emulated` encapsulation (omit the `encapsulation` property entirely). When you need to style dynamic HTML injected via `[innerHTML]`, use `:host ::ng-deep` in the component's stylesheet instead of disabling encapsulation.
